...
Review the current changes (`git diff` against `main_training`, plus untracked files under `blog/` and `authors/`).

## Checklist

### Structure
- Post at `blog/<category>/YYYY-MM-DD-slug/index.md`, category listed in `config.json`
- Frontmatter has `slug`, `title`, `authors`, `tags`; every author exists in `authors/authors.yml`
